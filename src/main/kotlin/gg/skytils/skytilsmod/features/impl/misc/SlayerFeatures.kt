...
object SlayerFeatures : CoroutineScope {
    ...
    // there might be a point replacing this with §c\d+:\d+(?:)?$ and only partially check for matches
    ...
    private val timerRegex =
        Regex("(?:§[8bef]§l(ASHEN|CRYSTAL|AURIC|SPIRIT)§[8bef] ♨\\d |§4§lIMMUNE )?§c\\d+:\\d+(?:)?")
                        ...
                        if (boss.startsWith("Revenant Horror")) {
                            BossStatus.setBossStatus(
                                RNGMeter(
                                    ...
                                    ChatComponentText("§2§lRevenant Horror RNG - §d${Skytils.config.revRNG}%")
                                ), true
                            )
                            break
                        }
                        if (boss.startsWith("Tarantula Broodfather")) {
                            BossStatus.setBossStatus(
                                RNGMeter(
                                    ...
                                    ChatComponentText("§8§lTarantula Broodfather RNG - §d${Skytils.config.taraRNG}%")
                                ), true
                            )
                            break
                        }
                        if (boss.startsWith("Sven Packmaster")) {
                            BossStatus.setBossStatus(
                                RNGMeter(
                                    ...
                                    ChatComponentText("§7§lSven Packmaster RNG - §d${Skytils.config.svenRNG}%")
                                ), true
                            )
                            break
                        }
                        if (boss.startsWith("Voidgloom Seraph")) {
                            BossStatus.setBossStatus(
                                RNGMeter(
                                    ...
                                    ChatComponentText("§5§lVoidgloom Seraph RNG - §d${Skytils.config.voidRNG}%")
                                ), true
                            )
                            break
                        }
                        if (boss.startsWith("Inferno Demonlord")) {
                            BossStatus.setBossStatus(
                                RNGMeter(
                                    ...
                                    ChatComponentText("§c§lInferno Demonlord RNG - §d${Skytils.config.blazeRNG}%")
                                ), true
                            )
                            break
                        }
                        if (boss.startsWith("Riftstalker Bloodfiend")) {
                            BossStatus.setBossStatus(
                                RNGMeter(
                                    ...
                                    ChatComponentText("§4§lRiftstalker Bloodfiend RNG - §d${Skytils.config.vampRNG}%")
                                ), true
                            )
                            break
                        }
    ...
    class SlayerDisplayElement : GuiElement("Slayer Display", x = 150, y = 20) {
        ...
        override val width: Int
            get() = ScreenRenderer.fontRenderer.getStringWidth("§c☠ §bRevenant Horror §a500§c❤")
