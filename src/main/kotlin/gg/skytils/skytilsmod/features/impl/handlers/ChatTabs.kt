...
object ChatTabs {
    ...
    enum class ChatTab(
        ...
        ALL("A"),
        PARTY("P", { _, formatted ->
            formatted.startsWith("§9Party") ||
                    formatted.startsWith("§9P §8> ") ||
                    formatted.endsWith("§ehas invited you to join their party!") ||
                    formatted.endsWith("§eto the party! They have §r§c60 §r§eseconds to accept.§r") ||
                    formatted == "§cThe party was disbanded because all invites expired and the party was empty§r" ||
                    formatted.endsWith("§ehas disbanded the party!§r") ||
                    formatted.endsWith("§ehas disconnected, they have §r§c5 §r§eminutes to rejoin before they are removed from the party.§r") ||
                    formatted.endsWith(" §ejoined the party.§r") ||
                    formatted.endsWith(" §ehas left the party.§r") ||
                    formatted.endsWith(" §ehas been removed from the party.§r") ||
                    formatted.startsWith("§eThe party was transferred to §r") ||
                    (formatted.startsWith("§eKicked §r") && formatted.endsWith("§e because they were offline.§r"))
        }),
        GUILD("G", { _, formatted ->
            formatted.startsWith("§2Guild > ") || formatted.startsWith("§2G > ")
        }),
        PRIVATE("PM", { _, formatted ->
            formatted.startsWith("§dTo ") || formatted.startsWith("§dFrom ")
        }),
        COOP("CC", { _, formatted ->
            formatted.startsWith("§bCo-op > ")
        });
