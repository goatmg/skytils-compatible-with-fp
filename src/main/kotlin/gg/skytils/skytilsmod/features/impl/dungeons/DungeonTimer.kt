...
object DungeonTimer {
    ...
    fun onChat(event: ClientChatReceivedEvent) {
        ...
        when {
            scoreShownAt == -1L && message.contains("§fTeam Score: §r") -> {
                scoreShownAt = System.currentTimeMillis()
            }

            (message == "§aStarting in 1 second.§r") && dungeonStartTime == -1L -> {
                dungeonStartTime = System.currentTimeMillis() + 1000
            }

            message.endsWith(" §ehas obtained §a§6§8Wither Key§e!") || unformatted == "A Wither Key was picked up!" || message.endsWith(
                "§ehas obtained §8Wither Key§e!"
            ) -> {
                witherDoors++
            }

            bloodOpenTime == -1L && (unformatted == "The BLOOD DOOR has been opened!" || message.startsWith(
                "§c[BOSS] The Watcher§f"
            )) -> {
            ...
            message == "§c[BOSS] The Watcher§f: You have proven yourself. You may pass." -> {
            ...
            bossEntryTime != -1L && bossClearTime == -1L && message.contains("§c☠ §r§eDefeated ") -> {
            ...
            Utils.equalsOneOf(DungeonFeatures.dungeonFloor, "F7", "M7") && (message.startsWith("§4[BOSS] ") || message.startsWith("§aThe Core entrance ")) -> {
                when {
                    message.endsWith("§cPathetic Maxor, just like expected.") && phase1ClearTime == -1L -> {
                    ...
                    message.endsWith("§cWho dares trespass into my domain?") && phase2ClearTime == -1L -> {
                    ...
                    message.endsWith(" is opening!") && terminalClearTime == -1L -> {
                    ...
                    message.endsWith("§cYou went further than any human before, congratulations.") && phase3ClearTime == -1L -> {
                    ...
                    message.endsWith("§cAll this, for nothing...") -> {
            ...
            Utils.equalsOneOf(DungeonFeatures.dungeonFloor, "F6", "M6") && message.startsWith("§c[BOSS] Sadan") -> {
                when {
                    (message.endsWith("§f: ENOUGH!§r") && terraClearTime == -1L) -> {
                    ...
                    (message.endsWith("§f: You did it. I understand now, you have earned my respect.") && giantsClearTime == -1L) -> {
