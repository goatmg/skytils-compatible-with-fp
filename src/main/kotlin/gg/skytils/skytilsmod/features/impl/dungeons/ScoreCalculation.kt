...
object ScoreCalculation {
    ...
    private val deathsTabPattern = Regex("§a§lDeaths: §f(?<deaths>\\d+)")
    private val missingPuzzlePattern = Regex("§b§lPuzzles: §f\\((?<count>\\d)\\)")
    private val failedPuzzlePattern =
        Regex(" (?<puzzle>.+): §7\\[§c§l✖§7] §.+")
    private val solvedPuzzlePattern =
        Regex(" (?<puzzle>.+): §7\\[§a§l✔§7] §.+")
    ...
    private val roomCompletedPattern = Regex(" Completed Rooms: §d(?<count>\\d+)")
    ...
    fun onTitle(event: MainReceivePacketEvent<*, *>) {
        ...
        if (event.packet.message.formattedText == "§eYou became a ghost!") {
