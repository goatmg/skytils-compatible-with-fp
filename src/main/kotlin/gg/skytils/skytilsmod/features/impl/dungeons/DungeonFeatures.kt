...
object DungeonFeatures {
        ...
        event.apply {
            if (hasBossSpawned && Skytils.config.spiritBearTimer && dungeonFloor?.endsWith('4') == true) {
                when (packet) {
                    ...
                    is S02PacketChat -> {
                        if (lastLitUpTime != -1L && packet.chatComponent.formattedText == "§a§lA §5§lSpirit Bear §a§lhas appeared!") {
        ...
        if (Utils.inDungeons) {
            ...
            if (isInTerracottaPhase && terracottaEndTime > 0 && Skytils.config.showSadanInterest) {
                ...
                if (timeLeft >= 0) {
                    ...
                    BossStatus.bossName = "§c§lSadan's Interest: §6" + timeLeft.toInt() + "s"
            ...
            if (equalsOneOf(unformatted, "Maxor", "Storm", "Goldor", "Necron")) {
                when (Skytils.config.necronHealth) {
                    2 -> {
                        ...
                        BossStatus.statusBarTime = 100
                        BossStatus.bossName = displayData.displayName.formattedText + "§8 - §d" + String.format(
                            "%.1f",
                    ...
                    1 -> {
                        ...
                        BossStatus.bossName = displayData.displayName.formattedText + "§8 - §a" + NumberUtil.format(
                            (BossStatus.healthScale * health).toLong()
                        ) + "§8/§a${NumberUtil.format(health)}§c❤"
                        BossStatus.hasColorModifier = event.hasColorModifier
                        event.isCanceled = true
                    }
            ...
            if (!mc.renderManager.isDebugBoundingBox) {
                if (!event.entity.isInvisible) {
                    ...
                    if (!hasBossSpawned && Skytils.config.boxStarredMobs && event.entity is EntityArmorStand && event.entity.hasCustomName() && event.entity.alwaysRenderNameTag) {
                        ...
                        // Define the patterns
                        val pattern1 = Pattern.compile("^§.\\[§.Lv\\d+§.\\] §.+ (?:§.)+0§f/.+§c❤$")
                        val pattern2 = Pattern.compile("^.+ (?:§.)+0§c❤$")

                        // Check if the name matches either pattern
                        if (pattern1.matcher(name).matches() || pattern2.matcher(name).matches()) {
                            // If it matches, do not render the box
                            return
                        }

                        // If entity is dead or health is zero, do not render the box
                        if (event.entity.isDead || event.entity.health <= 0) {
                            return
                        }
    ...
    internal class LividGuiElement : GuiElement("Livid HP", x = 0.05f, y = 0.4f) {
        ...
        override val height: Int
            get() = ScreenRenderer.fontRenderer.FONT_HEIGHT
        override val width: Int
            get() = ScreenRenderer.fontRenderer.getStringWidth("§f﴾ Livid §e6.9M§c❤ §f﴿")
