...
object DungeonListener {
    ...
    val partyCountPattern = Regex(" {9}§b§lParty §f\\((?<count>[1-5])\\)")
    private val classPattern =
        Regex("(?:§.)+(?:\\[.+] )?(?<name>\\w+?)(?:§.)* (?:(?:§[\\da-fklmno]){1,2}.+ )?§f\\(§d(?:(?<class>Archer|Berserk|Healer|Mage|Tank) (?<lvl>\\w+)|§7EMPTY)§f\\)")
    private val missingPuzzlePattern = Regex(" (?<puzzle>.+): §7\\[§6§l✦§7] ?")
    private val deathRegex = Regex("§c ☠ §7(?:You were |(?:§.)+(?<username>\\w+)).* and became a ghost§7\\.")
    private val reviveRegex = Regex("^§a ❣ §7(?:§.)+(?<username>\\w+)§a was revived")
        ...
        if (event.packet is S02PacketChat) {
            ...
            if (text == "§aStarting in 1 second.") {
                ...
                if (Skytils.config.dungeonDeathCounter) {
                    TickTask(6) {
                        UChat.chat("§c☠ §lDeaths: ${team.values.sumOf { it.deaths }}\n${
                            team.values.filter { it.deaths > 0 }.sortedByDescending { it.deaths }.joinToString("\n") {
                                "  §c☠ ${it.playerName}: ${it.deaths}"
                            }
                        }"
                        )
                    }
            ...
            } else if (text.startsWith("§c ☠ ") && text.endsWith(" and became a ghost§7.")) {
        ...
        TickTask(2, repeats = true) {
            if (Utils.inDungeons && (DungeonTimer.scoreShownAt == -1L || System.currentTimeMillis() - DungeonTimer.scoreShownAt < 1500)) {
                ...
                for (teammate in team.values) {
                    ...
                    if (self?.dead != true) {
                        if (entry.endsWith("§cDEAD§f)")) markDead(teammate)
    ...
    private fun getMembers() {
        ...
        if (tabEntries.isEmpty() || !tabEntries[0].second.contains("§b§lParty §f(")) {
            TickTask(5) {
                getMembers()
            }
            return
